    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          type="button"
          variant={"outline"}
          className={cn("flex gap-1 rounded-e-none rounded-s-lg px-3 text-black")}
          disabled={disabled}
        >
      ...
      <PopoverContent className="p-0 w-[300px] text-black" >
        <Command>
          <CommandList >
              ...
              {options
                ...
                .map((option) => (
                  ...
                  >
                    ...
                    <span className="text-sm flex-1 text-black">{option.label}</span>
                    {option.value && (
...
const FlagComponent = ({ country, countryName }: RPNInput.FlagProps) => {
  ...
  return (
    <span className="flex h-4 w-6 overflow-hidden rounded-sm bg-black/20">
